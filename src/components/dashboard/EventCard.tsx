...
export const EventCardContainer = ({
  ...
  return (
    <div
      className={cn('relative group/pin z-50  cursor-pointer w-fit')}
    ...
    >
        ...
        >
          <div className={cn(' relative z-50 ')}>
            <div className="flex basis-full flex-col p-4 tracking-tight text-slate-100/50 sm:basis-1/2 w-[20rem] h-[20rem] ">
              ...
              <p className="max-w-xs !pb-2 !m-0 font-bold  text-lg text-black">
                {title}
              </p>
              ...
              <div className="flex flex-1 w-full rounded-lg mt-4 overflow-hidden bg-gradient-to-br from-violet-500 via-purple-500 to-blue-500">
                {thumbnail && (
                  <Image
      ...
      <Link href={`/dashboard/${id}`} className="">
        <EventCardPerspective title={title} />
      </Link>
    </div>
  );
};

export const EventCardPerspective = ({ title }: { title?: string }) => {
  return (
      ...
      <div className=" w-full h-full -mt-7 flex-none  inset-0">
        <div className="absolute top-0 inset-x-0  flex justify-center">
          <div className="relative flex space-x-2 items-center z-50 rounded-full bg-zinc-950 py-0.5 px-4 ring-1 ring-white/10">
            <div className="relative z-20 text-white text-xs font-bold  py-0.5 flex flex-row gap-2">
              <div>{title}</div>
            </div>
